#include "rviz_3d_tools/goal_3d_tool.h"
...
void Goal3DTool::onInitialize() {
  ros::NodeHandle nh;
  pub_ = nh.advertise<geometry_msgs::PoseStamped>("/goal_3d", 1);

  arrow_ = new rviz::Arrow(scene_manager_, nullptr, 0.8f, 0.05f, 0.2f, 0.1f);
  arrow_->setColor(1.0f, 0.5f, 0.0f, 1.0f);
...
bool Goal3DTool::get3DPoint(rviz::ViewportMouseEvent& event, Ogre::Vector3& out) {
...
bool Goal3DTool::getDirectionPoint(rviz::ViewportMouseEvent& event, Ogre::Vector3& out) {
...
int Goal3DTool::processMouseEvent(rviz::ViewportMouseEvent& event) {
  ...
  if (event.leftUp()) {
    ...
    pub_.publish(msg);
    ROS_INFO("[Goal3DTool] Published goal: (%.2f, %.2f, %.2f) yaw=%.2f",
...
PLUGINLIB_EXPORT_CLASS(rviz_3d_tools::Goal3DTool, rviz::Tool)
